module Mobile::QuestionnaireHelper
  ...
  def self.freeze_questionnaire_replies_in_snapshot(quest_id, date = Time.now.strftime('%Y-%m-%d'))
    ...
    puts "Get all replies except for the test user"
    test_qpid = QuestionnaireParticipant.where(
      questionnaire_id: quest_id,
      employee_id: -1).last.id
    replies = QuestionReply
                .where(questionnaire_id: quest_id)
                .where.not(questionnaire_participant_id: test_qpid)
    ...
    qqids.each do |qqid|
      nid = QuestionnaireQuestion.find(qqid).network_id
      NetworkSnapshotData.where(snapshot_id: sid, network_id: nid).delete_all
    end

    ii = 0
    heading = "INSERT into network_snapshot_data (snapshot_id, original_snapshot_id, network_id, company_id, from_employee_id, to_employee_id, value, questionnaire_question_id) VALUES "
    sqlstr = heading
    ...
    begin
      replies.select { |reply| !reply.answer.nil? }.each do |reply|
        ...
        begin
          ii += 1
          if (ii % 200 == 0)
            puts "Insert block #{ii} out of #{num_replies}"
            ActiveRecord::Base.connection.exec_query(sqlstr[0..-2])
            sqlstr = heading
          end
          ...
          nid = qq.network_id
          raise "Question with ID: #{qq.id} without a netowrk" if nid.nil?
          ...
          if from.nil?
            msg = "Cant find from with id: #{reply.questionnaire_participant_id}"
            EventLog.log_event(event_type_name: 'ERROR', message: msg)
            puts msg
            next
          end
          if to.nil?
            msg = "Cant find from with id: #{reply.reffered_questionnaire_participant_id}"
            EventLog.log_event(event_type_name: 'ERROR', message: msg)
            puts msg
            next
          end

          if from.employee.nil?
            msg = "Canot find employee for participant: #{reply.questionnaire_participant_id}, at reply id: #{reply.id}"
            EventLog.log_event(event_type_name: 'ERROR', message: msg)
            puts msg
            next
          end
          if to.employee.nil?
            msg = "Canot find employee for participant: #{reply.reffered_questionnaire_participant_id}, at reply id: #{reply.id}"
            EventLog.log_event(event_type_name: 'ERROR', message: msg)
            puts msg
            next
          end

          sqlstr += "(#{sid}, #{sid}, #{nid}, #{cid}, #{from.employee.id}, #{to.employee.id},#{value},#{reply.questionnaire_question_id}),"
        rescue => e
          msg = "
            EXCEPTION: Failed to freeze. reply id: #{reply.id} qid: #{qq.id}, sid: #{sid}, qqid: #{qqid},
            from: #{reply.questionnaire_participant_id}, to: #{reply.reffered_questionnaire_participant_id},
      ...
      msg = "Finished freezing questionnaire: #{quest_id}, to snapshot: #{sid}"
      EventLog.log_event(event_type_name: EVENT_TYPE, message: msg)
      puts msg
