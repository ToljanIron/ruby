module SelectionHelper
  ...
  def get_inner_select_as_arr(cid, pinid, gid)
    return get_inner_select_by_group_as_arr(gid) if pinid == NO_PIN && gid != NO_GROUP
    return get_inner_select_by_pin_as_arr(pinid)    if pinid != NO_PIN && gid == NO_GROUP
  ...
  def format_from_activerecord_result(f_in_n)
    ...
    f_in_n.rows.each do |row|
      ret << { id: row[ID].to_i, measure: row[MEASURE].to_s }
    end
    return ret
  end
end
