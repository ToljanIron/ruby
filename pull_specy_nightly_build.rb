...
class PullHelper
  ...
  def pull
    ...
    @envs.each do |e|
      ...
      unless target
        puts "nothing to fetch for #{e}, master is the lastest build".green
        next
      end
      `git fetch #{@remote} #{target}:#{target}`
      fail 'failed to fetch #{target}' unless $CHILD_STATUS.success?
      puts "fetched #{target}".green
      puts 'Notice! this branch was created more then 24h ago, did specy fail to build last night?'.yellow if old_branch? target
    end
  end
