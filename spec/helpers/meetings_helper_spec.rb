...
describe MeetingsHelper, type: :helper do
  ...
  describe 'create_meeting_attendees' do
    before do
      ...
      @snapshot = create(:snapshot)
      create(:meeting, meeting_uniq_id: 'abcd', snapshot_id: @snapshot.id)
    end
    ...
    it 'should create meeting attendees for emps' do
      ...
      MeetingsHelper.create_attendees('abcd': ['email1@company.com', 'email2@company.com', 'email3@company.com'])
      expect(MeetingAttendee.count).to eq(3)
      expect(MeetingAttendee.pluck(:attendee_type)).to eq([0, 0, 0])
      expect(MeetingAttendee.first[:attendee_id]).to eq(1)
      expect(MeetingAttendee.second[:attendee_id]).to eq(2)
      expect(MeetingAttendee.last[:attendee_id]).to eq(3)
    end
    ...
    it 'should create meeting attendees for emps if given aliases' do
      ...
      MeetingsHelper.create_attendees('abcd': ['e1@company.com'])
      expect(MeetingAttendee.count).to eq(1)
      expect(MeetingAttendee.first[:attendee_id]).to eq(1)
      expect(MeetingAttendee.first[:attendee_type]).to eq(0)
    end
    ...
    it 'should create meeting attendees for external domains if they exist' do
      ...
      MeetingsHelper.create_attendees('abcd': ['first@domain.com'])
      expect(MeetingAttendee.count).to eq(1)
      expect(MeetingAttendee.first[:attendee_id]).to eq(1)
      expect(MeetingAttendee.first[:attendee_type]).to eq(1)
    end
